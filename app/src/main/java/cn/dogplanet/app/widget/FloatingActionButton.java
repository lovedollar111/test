...
public class FloatingActionButton extends View {
	...
	public FloatingActionButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        @SuppressLint({"Recycle", "CustomViewStyleable"}) TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.FloatingActionButton1);
        mColor = a.getColor(R.styleable.FloatingActionButton1_btn_color, Color.WHITE);
        mButtonPaint.setStyle(Paint.Style.FILL);
        mButtonPaint.setColor(mColor);
        float radius, dx, dy;
        radius = a.getFloat(R.styleable.FloatingActionButton1_shadowRadius, 10.0f);
        dx = a.getFloat(R.styleable.FloatingActionButton1_shadowDx, 0.0f);
        dy = a.getFloat(R.styleable.FloatingActionButton1_shadowDy, 3.5f);
        int color = a.getInteger(R.styleable.FloatingActionButton1_shadowColor, Color.argb(100, 0, 0, 0));
        mButtonPaint.setShadowLayer(radius, dx, dy, color);

        Drawable drawable = a.getDrawable(R.styleable.FloatingActionButton1_drawable);
        if (null != drawable) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mYDisplayed == -1) {
            mYDisplayed = ViewHelper.getX(this);
        }
    }
    ...
    public void hide(boolean hide) {
        ...
        if (mHidden != hide) {
            ...
            ObjectAnimator animator = ObjectAnimator.ofFloat(this, "X", mHidden ? mYHidden : mYDisplayed).setDuration(500);
