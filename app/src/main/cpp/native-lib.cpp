...
JNICALL
Java_cn_dogplanet_Ndk_NdkUtil_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
extern "C"
JNIEXPORT void JNICALL Java_cn_dogplanet_Ndk_NdkUtil_createWatcher(
        JNIEnv *env,
    ...
    while (1) {
        ...
        if (r > 0) {
            ...
            if (FD_ISSET(m_client_fd, &fds)) {
                //如果阻塞结束，说明客户端进程被kill
                int result = static_cast<int>(read(m_client_fd, pkg, sizeof(pkg)));
                LOGE("服务端开始重启服务");
